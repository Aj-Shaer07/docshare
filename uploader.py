...
"""
uploader.py
Wi-Fi (TCP) TLS file uploader. Mutual TLS required.
...
async def send_file(host, port, filepath, cert, key, ca):
    ...
    meta = {"filename": filename, "filesize": filesize, "chunk_size": CHUNK_SIZE,
            "file_sha256": file_sha, "transfer_id": transfer_id}
    ...
    offset = json.loads(hdr_raw.decode()).get("resume_offset",0)
...
def main():
    ...
    while True:
        ...
        print(f"File '{filepath}' does not exist. Try again.")
